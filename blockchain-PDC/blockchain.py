...
class Block:
    ...
    def create_hash(self):
        blockstring = (
            f"{self.index}{self.prior_hash}{self.timestamp}{self.data}".encode()
        )
...
if __name__ == "__main__":
    ...
    print(
        "Blockchain apos alteracao de dados valida?"
        + str(student_coin.valiadating_blockchain())
    )

    student_coin.chain[1].hash = student_coin.chain[1].create_hash()
    print(
        "Blockchain apos atualizar o hash valida?"
        + str(student_coin.valiadating_blockchain())
    )
